...
for filename in sorted(glob.glob('best_birds_scores/mean_scores*.txt'), key=lambda x:float(re.findall("(\d+)",x)[0])):
    mean_scores.append(np.loadtxt(filename))

for filename in sorted(glob.glob('best_birds_scores/scores*.txt'), key=lambda x:float(re.findall("(\d+)",x)[0])):
