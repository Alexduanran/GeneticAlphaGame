...
class Main():
    ...
    def next_generation(self):
        ...
        if self.current_generation <= 100:
            ...
            self.save_bird('Flappy_Genetic/plot/best_birds_each_generation', individual_name, self.winner, settings)    
    ...
    def save_bird(self, population_folder: str, individual_name: str, bird: Bird, settings: Dict[str, Any]) -> None:
        ...
        L = len(bird.network.layer_nodes)
        for l in range(1, L):
            w_name = 'W' + str(l)
            b_name = 'b' + str(l)

            weights = bird.network.params[w_name]
            bias = bird.network.params[b_name]
